...
namespace WebSystem.Repository
{
    public class CustomerRepo
    {
       ...
       public void insertRefPerson(string name,string adrs,string bank,int accno)
        ...
        {
            var name = (from x in db.AccRefPersons where x.FullName.Contains(prefix) select new { Name = x.FullName, ID = x.ReferancePersonID});

            return name;
        }

        public dynamic getDetails(int id)
        {
            var details = (from x in db.AccRefPersons where x.ReferancePersonID.Equals(id) select x).ToList();
            return details;
        }
    }
}